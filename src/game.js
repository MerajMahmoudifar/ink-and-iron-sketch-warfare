...
class Combat {
  ...
  static getForecast(attacker, defender, grid, factions) {
    ...
    const dist = Combat.getDistance(attacker, defender);
...
class GameEngine {
    ...
    this.timerInterval = setInterval(() => {
      ...
      if (this.phase === GAME_PHASES.PLANNING) {
        ...
        if (this.planningTimeRemaining <= 5 && this.planningTimeRemaining > 0) {
          try { if (this.audio) this.audio.playCountdownTick(); } catch(err){}
        }
  ...
  endPlanningPhase() {
    ...
    this.playbackTimeRemaining = this.playbackDurationConfig;
    this.currentPlaybackStep = 0;
    try { if (this.audio) this.audio.playActionWhistle(); } catch(err){}
      ...
      p2Units.forEach(u2 => {
        ...
        if (dist <= Math.max(u1.attackRange, u2.attackRange)) {
          ...
          if (u1CanAttack) {
            ...
            if (this.audio) {
              if (u1.id === 'HEAVY_SIEGE_TANK' || u2.id === 'HEAVY_SIEGE_TANK') this.audio.playTankCannon();
  ...
  async preloadAll() {
    ...
    const loadPromises = Object.entries(this.audioManifest).map(async ([key, url]) => {
      try {
        ...
        // Fallback: If asset cannot be fetched, procedural fallback is ready
      }
    });
...
class UIManager {
  ...
  setupListeners() {
    ...
    document.getElementById('btn-menu-logout')?.addEventListener('click', (e) => {
      ...
      try { this.app.audio.playSwitch(); } catch(err){}
    });
  ...
  updateHUD(engine) {
    ...
    try { this.updateActionLogs(engine); } catch(e){ console.error('Error updating action logs:', e); }
      ...
      try {
        ...
        if (modal) {
          ...
          if (this.app && this.app.audio) {
            if (isVictory) this.app.audio.playVictorySound();
            else this.app.audio.playDefeatSound();
          }
        ...
        console.error('Error displaying victory modal:', err);
        const modal = document.getElementById('victory-modal');
        if (modal) modal.style.display = 'flex';
        if (this.app && this.app.audio) {
          if (engine.winner === 1) this.app.audio.playVictorySound();
          else this.app.audio.playDefeatSound();
        }
      }
    }
    ...
    if (forecastCard) {
      ...
      if (unitOnTile) {
        if (unitOnTile.owner === 2) {
          ...
          }) || friendlyUnits.find(u => Combat.getDistance(u, unitOnTile) <= u.attackRange);

          if (!bestAttacker && friendlyUnits.length > 0) {
            bestAttacker = friendlyUnits.reduce((closest, cur) => {
              const dCur = Combat.getDistance(cur, unitOnTile);
              const dClo = Combat.getDistance(closest, unitOnTile);
          ...
          const targetEnemy = enemyUnits.find(e => Combat.getDistance(unitOnTile, e) <= unitOnTile.attackRange) ||
                              (enemyUnits.length > 0 ? enemyUnits.reduce((closest, cur) => {
                                ...
                                const dClo = Combat.getDistance(unitOnTile, closest);
                                return dCur < dClo ? cur : closest;
                              }, enemyUnits[0]) : null);

          if (targetEnemy) {
            forecast = Combat.getForecast(unitOnTile, targetEnemy, engine.grid, factionsMap);
          }
        }
      }
...
window.applyCustomSpeed = function() {
  ...
  const modal = document.getElementById('custom-speed-modal');
  if (modal) modal.style.display = 'none';
  try { if (window.gApp && window.gApp.audio) window.gApp.audio.playStamp(); } catch(e) {}
};
...
window.setCustomChip = function(field, val) {
  ...
  window._updateCustomSpeedSummary();
  window._updateCustomChipActiveStates();
  try { if (window.gApp && window.gApp.audio) window.gApp.audio.playTick(); } catch(e) {}
};
